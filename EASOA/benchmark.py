...
def F7_function(x):
    ...
    x_pow_4 = x**4
    sum_part = np.sum(x_pow_4 * i_vector, axis=1)
...
# F12 helper function
def _u(xi, a, k, m):
    u_vals = np.zeros_like(xi)

    # Condition 1: xi > a
    cond1_mask = xi > a
    u_vals[cond1_mask] = k * (xi[cond1_mask] - a)**m
...
def F18_function(x):
    ...
    n_pop = x.shape[0]
    x_reshaped = x.reshape(n_pop, 1, 3)
    P_reshaped = P.reshape(1, 4, 3)
    A_reshaped = A.reshape(1, 4, 3)
    diff = x_reshaped - P_reshaped
    diff_sq = diff**2
...
def F19_function(x):
    ...
    n_pop = x.shape[0]
    x_reshaped = x.reshape(n_pop, 1, 6)
